...
class CameraApp(QWidget):
    ...
    def UiConfig(self):
        ...
        self.picBox.addItem("yolov8-improve-engine-py")
        ...
        self.capBox.addItem("yolov8-improve-engine-py")
    ...
    def LoadModel(self):
        ...
        self.Yolov8n = YOLO("./orah.pt")
        self.Yolov8n_improve = YOLO("./orah-im.pt")
        self.Yolov8n_improve_engine = YOLO("./orah-im-py.engine")
       
        self.det = Detector("/home/jetson/Desktop/CJS/tensorrt-yolov8/build/libyolov8_detect.so")
    ...
    def yolov8n_frame(self):
        ...
        for i in range(size):
            ...
            cv2.putText(frame, "Orah:"+conf+" dist:"+str(dist), (x1-5, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (Conf, 255-Conf, 255-Conf*0.5), 2 )
        self.fps = (self.fps + (1. / (time.time() - t1))) / 2
    ...
    def yolov8n_improve_frame(self):
        ...
        for i in range(size):
            ...
            dist = round(dist, 2)
            cv2.rectangle(frame, (x1, y1), (x2, y2), (Conf, 255-Conf, 255-Conf*0.5), 2)
            cv2.putText(frame, "Orah:" + conf + " dist:" + str(dist), (x1 - 5, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
    ...
    def yolov8n_pic(self):
        ...
        for i in range(size):
            ...
            cv2.rectangle(frame, (x1, y1), (x2, y2), (Conf, 255-Conf, 255-Conf*0.5), 2)
            cv2.putText(frame, "Orah:"+conf, (x1-5, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (Conf, 255-Conf, 255-Conf*0.5), 2)
        frame = cv2.putText(frame, " Count:"+str(count), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # 将OpenCV的图像格式转换为Qt图像格式
        height, width, channel = frame.shape
        bytes_per_line = 3 * width
    ...
    def yolov8n_improve_pic(self):
        ...
        t = str(t)  # 转换为字符串
        t2 = t + "ms"
        self.timeLabel.setText("RunTime: "+t2)
        result = results[0]
        size = result.boxes.shape[0]
        for i in range(size):
            count = count + 1
            ...
            cv2.rectangle(frame, (x1, y1), (x2, y2), (Conf, 255-Conf, 255-Conf*0.5), 2)
            cv2.putText(frame, "Orah:" + conf, (x1 - 5, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (Conf, 255-Conf, 255-Conf*0.5), 2)
        frame = cv2.putText(frame, " Count:"+str(count), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            
 #           fps = 1000 / float(t)
 #           fps = round(fps, 2)
 #           frame = cv2.putText(frame, "FPS:"+str(fps)+" Count:"+str(count), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)      
        # 将OpenCV的图像格式转换为Qt图像格式
        height, width, channel = frame.shape
        bytes_per_line = 3 * width
        qt_image = QImage(frame.data, width, height, bytes_per_line, QImage.Format_RGB888).rgbSwapped()
